...
import { FiEdit3, FiEye, FiPlus, FiSearch, FiTrash2, FiX, FiMonitor, FiClock, FiActivity, FiFolder, FiBookOpen, FiUser, FiShield, FiExternalLink, FiSettings } from "react-icons/fi";
...
export default function ManageResource({ type }) {
  ...
  const [adminActionModal, setAdminActionModal] = useState(null); // 'suspend' | 'block' | 'reset-password'
  ...
  const handleSuspendToggle = async (user, shouldSuspend) => {
    if (shouldSuspend) {
      setAdminActionModal("suspend");
      setActionReason("");
    } else {
      if (!window.confirm("Are you sure you want to unsuspend this user?")) return;
      setActionLoading(true);
      try {
        const { data: res } = await api.patch(`/admin/users/${user._id}/suspend`, { suspend: false });
        setUserDetail(prev => ({ ...prev, user: res.data }));
        setRows(prev => prev.map(r => r._id === user._id ? { ...r, ...res.data } : r));
        pushToast("User unsuspended successfully.", "success");
      } catch (err) {
        pushToast(err.message || "Failed to unsuspend user.", "error");
      } finally {
  ...
  const handleBlockToggle = async (user, shouldBlock) => {
    if (shouldBlock) {
      setAdminActionModal("block");
      setActionReason("");
    } else {
      if (!window.confirm("Are you sure you want to unblock this user?")) return;
      setActionLoading(true);
      try {
        const { data: res } = await api.patch(`/admin/users/${user._id}/block`, { block: false });
        setUserDetail(prev => ({ ...prev, user: res.data }));
        setRows(prev => prev.map(r => r._id === user._id ? { ...r, ...res.data } : r));
        pushToast("User unblocked successfully.", "success");
      } catch (err) {
        pushToast(err.message || "Failed to unblock user.", "error");
      } finally {
        setActionLoading(false);
      }
    }
  ...
  const handleResetPasswordPrompt = (user) => {
    setAdminActionModal("reset-password");
    setNewPasswordValue("");
  };
  ...
  const handleAdminActionSubmit = async (e) => {
    ...
    try {
      ...
      if (adminActionModal === "suspend") {
        const { data: res } = await api.patch(`/admin/users/${userId}/suspend`, { suspend: true, reason: actionReason });
        setUserDetail(prev => ({ ...prev, user: res.data }));
        setRows(prev => prev.map(r => r._id === userId ? { ...r, ...res.data } : r));
        pushToast("User suspended successfully.", "success");
      } else if (adminActionModal === "block") {
        const { data: res } = await api.patch(`/admin/users/${userId}/block`, { block: true, reason: actionReason });
        setUserDetail(prev => ({ ...prev, user: res.data }));
        setRows(prev => prev.map(r => r._id === userId ? { ...r, ...res.data } : r));
        pushToast("User blocked successfully.", "success");
      } else if (adminActionModal === "reset-password") {
        await api.patch(`/admin/users/${userId}/reset-password`, { password: newPasswordValue });
        pushToast("Password reset completed successfully.", "success");
      }
      setAdminActionModal(null);
      setActionReason("");
      setNewPasswordValue("");
    } catch (err) {
      pushToast(err.message || "Failed to execute administrative action.", "error");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  return (
    <section>
          ...
          >
            ...
            {userDetailLoading ? (
              ...
              <div className="p-6 space-y-6">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      {userDetail.user?.isBlocked ? (
                        <span className="inline-block mt-1 px-2 py-0.5 text-xs font-bold rounded-full bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border border-red-200 dark:border-red-900/20">Blocked</span>
                      ) : userDetail.user?.isSuspended ? (
                        <span className="inline-block mt-1 px-2 py-0.5 text-xs font-bold rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-900/20">Suspended</span>
                      ) : (
                        <span className="inline-block mt-1 px-2 py-0.5 text-xs font-bold rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-200 dark:border-green-900/20">Active</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {userDetail.user?.isBlocked && (
                  <div className="rounded-xl border border-red-200 dark:border-red-950 p-4 bg-red-50/20 dark:bg-red-950/10 text-sm text-red-700 dark:text-red-400">
                    <strong>Block Reason:</strong> {userDetail.user?.blockReason || "No reason specified."}
                  </div>
                )}

                {userDetail.user?.isSuspended && (
                  <div className="rounded-xl border border-amber-200 dark:border-amber-950 p-4 bg-amber-50/20 dark:bg-amber-950/10 text-sm text-amber-700 dark:text-amber-400">
                    <strong>Suspension Reason:</strong> {userDetail.user?.suspensionReason || "No reason specified."}
                  </div>
                )}

                {/* Administrative Controls */}
                <div className="rounded-xl border border-rose-200 dark:border-rose-950/40 p-5 bg-rose-50/10 dark:bg-rose-950/5">
                  <h3 className="text-sm font-bold text-rose-600 dark:text-rose-450 uppercase tracking-wider flex items-center gap-2">
                    <FiSettings size={14} /> Administrative Controls
                  </h3>
                  <div className="mt-4 flex flex-wrap gap-2.5">
                    {userDetail.user?.isSuspended ? (
                      <Button
                        variant="outline"
                        onClick={() => handleSuspendToggle(userDetail.user, false)}
                        className="border-amber-300 text-amber-600 dark:text-amber-450 hover:bg-amber-50 dark:hover:bg-amber-950/30"
                      >
                        Unsuspend User
                      </Button>
                    ) : (
                      <Button
                        variant="outline"
                        onClick={() => handleSuspendToggle(userDetail.user, true)}
                        className="border-amber-300 text-amber-600 dark:text-amber-450 hover:bg-amber-50 dark:hover:bg-amber-950/30"
                      >
                        Suspend User
                      </Button>
                    ...
                    {userDetail.user?.isBlocked ? (
                      <Button
                        variant="outline"
                        onClick={() => handleBlockToggle(userDetail.user, false)}
                        className="border-red-300 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30"
                      >
                        Unblock User
                      </Button>
                    ) : (
                      <Button
                        variant="outline"
                        onClick={() => handleBlockToggle(userDetail.user, true)}
                        className="border-red-300 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30"
                      >
                        Block User
                      </Button>
                    ...
                    <Button
                      variant="outline"
                      onClick={() => handleResetPasswordPrompt(userDetail.user)}
                      className="border-indigo-300 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/30"
                    >
                      Reset Password
                    </Button>
                  </div>
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-5">
                  <h3 className="text-sm font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider flex items-center gap-2">
                    <FiShield size={14} /> Verification Files ({userDetail.verifications?.length || 0})
                  </h3>
                  <div className="mt-3 space-y-2">
                    {userDetail.verifications?.length > 0 ? userDetail.verifications.map((doc) => (
                      <div key={doc._id} className="rounded-lg bg-slate-50 dark:bg-slate-950 p-3 text-sm flex items-center justify-between">
                        <div>
                          <p className="font-semibold text-slate-800 dark:text-slate-200">{doc.documentType}</p>
                          <p className="text-xs text-slate-500">
                            Submitted: {new Date(doc.createdAt).toLocaleDateString()} · Status: <span className="font-semibold">{doc.status}</span>
                          </p>
                        </div>
                        <a
                          href={doc.file.url}
                      ...
                      <p className="text-xs text-slate-500">No verification documents uploaded.</p>
                    )}
                  </div>
                </div>
      ...
      {/* Admin Action Modal Dialog Overlay */}
      {adminActionModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-slate-200 dark:border-slate-800 bg-white p-6 dark:bg-slate-900 shadow-2xl space-y-4 animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between border-b pb-3 dark:border-slate-800">
              <h3 className="text-lg font-bold text-slate-950 dark:text-white capitalize">
                {adminActionModal === "reset-password" ? "Reset Password" : `${adminActionModal} Account`}
              </h3>
              <button onClick={() => setAdminActionModal(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                <FiX size={20} />
              </button>
            </div>

            <form onSubmit={handleAdminActionSubmit} className="space-y-4">
              {adminActionModal === "reset-password" ? (
                <div>
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 block mb-1">
                    Reason for {adminActionModal}
                  </label>
                  <textarea
                    value={actionReason}
                    onChange={(e) => setActionReason(e.target.value)}
                    placeholder={`Specify the reason for this ${adminActionModal} action...`}
                    className="focus-ring w-full min-h-20 rounded-lg border border-slate-300 px-3 py-2 dark:border-slate-700 dark:bg-slate-950 dark:text-white text-sm"
                    required
                  />
                </div>
              )}
              ...
              <div className="flex justify-end gap-2 border-t pt-3 dark:border-slate-800">
                <Button type="button" variant="ghost" onClick={() => setAdminActionModal(null)} disabled={actionLoading}>
                  Cancel
                </Button>
